...
app.get('/hospitaldetails',middleware.checkToken, function(req,res){
    ...
    var data=db.collection('hospInfo').find().toArray().then(result=>res.json(result));
});

app.get('/ventilatordetails',middleware.checkToken,(req,res)=>{
    console.log("Ventilator Information");//ventilator info
    var ventilatordetails=db.collection('ventInfo').find().toArray().then(result=>res.json(result));
});

app.post('/searchventbystatus',middleware.checkToken,(req,res)=>{
    var status=req.body.status;//by status searching ventilators
    console.log(status);
    var ventilatordetails=db.collection('ventInfo').find({"status":status}).toArray().then(result=>res.json(result));
});

app.post('/searchventbyname',middleware.checkToken,(req,res)=>{
    var name=req.query.name;//by hospital name searching ventilators
    console.log(name);
    var ventilatordetails=db.collection('ventInfo').find({'name':new RegExp(name,'i')}).toArray().then(result=>res.json(result));
});

app.post('/searchhospital',middleware.checkToken,(req,res)=>{
    var name=req.body.name;//search hospital by name
    console.log(name);
    var hospitaldetails=db.collection('hospInfo').find({'name':new RegExp(name,'i')}).toArray().then(result=>res.json(result));
});
...
app.put('/updateventilator',middleware.checkToken,(req,res)=>{
    ...
    db.collection("ventInfo").updateOne(ventid,newvalues,function(err,result){
...
app.post('/addventilatorbyuser',middleware.checkToken,(req,res)=>{
    ...
    db.collection('ventInfo').insertOne(item,function(err,result){
